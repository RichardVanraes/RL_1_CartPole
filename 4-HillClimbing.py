...
for _ in range(nr_of_iterations):
    intermediate_history = []
    weights = best_weights + noise_scaling * (2*np.random.rand(4)-1.0) # Hill Climbing
...
df_scaling = pd.DataFrame(np.row_stack(climbing_weights), columns=['cart pos','cart velocity','angle','angular velocity'])
df_scaling['history'] = climbing_history
...
sns.displot(data=climbing_history)
# plt.show()
