...
# Range to cover the whole state space is -1 to 1 = 2 by addition of noise
# Cover 2 in 1000 iterations = 0.002 per iteration
# if noise_scaling = 0.01 then random spread should be 0.2
noise_scaling = 0.2
spread = 1
...
for _ in range(nr_of_iterations):
    episode_history = []
    weights = best_weights + noise_scaling * np.random.normal(loc=0, scale=spread, size=4) # Hill Climbing
    ...
    climbing_weights.append(weights)
